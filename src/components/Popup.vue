...
<template>
  <div class="fixed w-full h-screen z-50 flex justify-center items-center inset-0 bg-gray-500/[47%]" @click.self="closePopup">
    <div class="w-80 min-h-[150px] rounded-[26px] bg-gray-300 shadow-normal flex flex-col justify-evenly items-center">
      ...
      <textarea v-else placeholder="輸入文字" class="h-[84px] bg-white w-[85%] rounded-2xl my-5 p-2" v-model.trim="remark"></textarea>
      ...
      <button v-else class="btn btn-green mx-auto h-14 w-56" @click="closePopup">確定</button>
    </div>
  </div>
</template>
